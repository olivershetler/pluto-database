...
SELECT 
SUM(CASE WHEN pcvpu_target < 0 THEN 1 ELSE 0 END) AS "below-0%"
--,SUM(CASE WHEN pcvpu_target >= -5 AND pcvpu_target < 0 THEN 1 ELSE 0 END) AS "-5-0%"
,SUM(CASE WHEN pcvpu_target >= 0 AND pcvpu_target < 5 THEN 1 ELSE 0 END) AS "0%-5%"
--,SUM(CASE WHEN pcvpu_target >= 4 AND pcvpu_target < 6 THEN 1 ELSE 0 END) AS "4%-6%"
,SUM(CASE WHEN pcvpu_target >= 5 AND pcvpu_target < 10 THEN 1 ELSE 0 END) AS "5%-10%"
,SUM(CASE WHEN pcvpu_target >= 10 THEN 1 ELSE 0 END) AS "above-10%"
FROM pluto_sym;

DROP TABLE pluto_disc;
CREATE TABLE pluto_disc (
    -- keys
    year                        NUMERIC
    ,bbl                        NUMERIC
    ,block                      NUMERIC
    ...
    -- the target
    ,pcvpu_target                   VARCHAR NOT NULL
);

INSERT INTO pluto_disc
SELECT
    year::NUMERIC   
    ,bbl::NUMERIC
    ,block::NUMERIC
    ...
    ,avg_zip_pcvpu_L3
    -- target
    ,(CASE WHEN pcvpu_target < 0 THEN 0
      WHEN pcvpu_target >= 0 AND pcvpu_target < 5 THEN 1
      WHEN pcvpu_target >= 5 AND pcvpu_target < 10 THEN 2
      WHEN pcvpu_target >= 10 THEN 3
      ELSE -100 END) AS pcvpu_target
FROM pluto_sym;